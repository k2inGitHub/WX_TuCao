...
namespace CSVParser {
    ...
    void Csv::Load(const string& filename, string& Data)
    {
        ...
        char filePath[200] = { 0 };
        sprintf(filePath, "%s/%s",tempStr.c_str(),filename.c_str());
...
#else
        ssize_t nSize;
        const char *pXmlBuffer = (const char*)FileUtils::getInstance()->getFileData(filename, "rb", &nSize);
        if (NULL == pXmlBuffer) {
            log("readInfo fail");
            return;
        }
        Data.assign(pXmlBuffer, nSize);
        delete[] pXmlBuffer;
#endif
    }
...
//        CCLOG("----------------------text = %s",text.c_str());
        ...
        for (unsigned long i = 0, size = text.size(); i < size; ++i)
        {
